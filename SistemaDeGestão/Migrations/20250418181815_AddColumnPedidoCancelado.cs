using System;
using Microsoft.EntityFrameworkCore.Migrations;

#nullable disable

namespace SistemaDeGestão.Migrations
{
    /// <inheritdoc />
    public partial class AddColumnPedidoCancelado : Migration
    {
        /// <inheritdoc />
        protected override void Up(MigrationBuilder migrationBuilder)
        {


            migrationBuilder.CreateTable(
                name: "PedidosCancelados",
                columns: table => new
                {
                    Id = table.Column<int>(type: "int", nullable: false)
                        .Annotation("SqlServer:Identity", "1, 1"),
                    PedidoId = table.Column<int>(type: "int", nullable: false),
                    DataCancelamento = table.Column<DateTime>(type: "datetime2", nullable: false),
                    MotivoCancelamento = table.Column<string>(type: "nvarchar(max)", nullable: false),
                    CodigoReembolso = table.Column<string>(type: "nvarchar(max)", nullable: false),
                    EstaReembolsado = table.Column<bool>(type: "bit", nullable: false),
                    ValorReembolsado = table.Column<decimal>(type: "decimal(18,2)", nullable: false),
                    TransacaoReembolsoId = table.Column<string>(type: "nvarchar(max)", nullable: false),
                    FinalUserId = table.Column<int>(type: "int", nullable: false)
                },
                constraints: table =>
                {
                    table.PrimaryKey("PK_PedidosCancelados", x => x.Id);
                    table.ForeignKey(
                        name: "FK_PedidosCancelados_FinalUsers_FinalUserId",
                        column: x => x.FinalUserId,
                        principalTable: "FinalUsers",
                        principalColumn: "Id",
                        onDelete: ReferentialAction.Cascade);
                    table.ForeignKey(
                        name: "FK_PedidosCancelados_Pedidos_PedidoId",
                        column: x => x.PedidoId,
                        principalTable: "Pedidos",
                        principalColumn: "Id",
                        onDelete: ReferentialAction.Cascade);
                });

            migrationBuilder.CreateIndex(
                name: "IX_PedidosCancelados_FinalUserId",
                table: "PedidosCancelados",
                column: "FinalUserId");

            migrationBuilder.CreateIndex(
                name: "IX_PedidosCancelados_PedidoId",
                table: "PedidosCancelados",
                column: "PedidoId");
        }

        /// <inheritdoc />
        protected override void Down(MigrationBuilder migrationBuilder)
        {
            migrationBuilder.DropTable(
                name: "PedidosCancelados");
        }
    }
}
